...
import {MorphoBalancesLib} from "src/libraries/MorphoBalancesLib.sol";
...
contract BaseTest is Test {
        ...
        returns (uint256, uint256, uint256)
    {
        ...
        if (minCollateral <= MAX_COLLATERAL_ASSETS) {
            ...
            amountBorrowed = Math.min(amountBorrowed.wMulDown(marketParams.lltv).mulDivDown(priceCollateral, ORACLE_PRICE_SCALE), MAX_TEST_AMOUNT);
        }
    ...
    function _boundUnhealthyPostion(uint256 amountCollateral, uint256 amountBorrowed, uint256 priceCollateral)
        internal
        view
        returns (uint256, uint256, uint256)
    {
...

    {
        ...
        return bound(assets, 0, collateral.zeroFloorSub(borrowed.wDivUp(_marketParams.lltv).mulDivUp(ORACLE_PRICE_SCALE, collateralPrice)));
    }
        ...
        returns (uint256)
    {
        ...
        return bound(assets, 0, MAX_TEST_AMOUNT.toSharesDown(morpho.totalSupplyAssets(_id), morpho.totalSupplyShares(_id)).zeroFloorSub(supplyShares));
    }
