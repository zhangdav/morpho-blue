    ...
    IMorphoSupplyCollateralCallback
{
    ...
    function testSupplyCollateralCallback(uint256 amount) public {
        ...
        morpho.supplyCollateral(marketParams, amount, address(this), abi.encode(this.testSupplyCollateralCallback.selector, hex""));
    }
    ...
    function testFlashActions(uint256 loanAmount) public {
        ...
        assertGe(morpho.borrowShares(marketParams.id(), address(this)), 0, "no borrow");
    ...
    function testFlashLoanShouldRevertIfNotReimbursed(uint256 amount) public {
        ...
        morpho.flashLoan(address(loanToken), amount, abi.encode(this.testFlashLoanShouldRevertIfNotReimbursed.selector, hex""));
    }

    function testLiquidateCallback(uint256 seized) public {
        uint256 loanAmount = 10e18;
        uint256 collateralAmount = 20e18;
        
        vm.assume(seized >= 5e18);
        vm.assume(seized <= collateralAmount);
        
        seized = bound(seized, 5e18, collateralAmount);
        ...
        oracle.setPrice(0.6e18);

        loanToken.setBalance(address(this), loanAmount);
        loanToken.approve(address(morpho), 0);

        vm.expectRevert();
        morpho.liquidate(marketParams, address(this), seized, 0, hex"");
        morpho.liquidate(marketParams, address(this), seized, 0, abi.encode(this.testLiquidateCallback.selector, hex""));
    }
}